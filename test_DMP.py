    ...
    def __init__(self, N, tau, dt, Dof, scale):
        ...
        for j in range(0, self.Dof):
            ...
            for i in range(0, self.N):
                ...
                grad[:, i * self.Dof + 4 + j] = DMP_layer.integrate(data, weights, 0, 0, 0, self.tau)

            # start_point
            weights = torch.zeros((self.N))
            grad[:, j] = DMP_layer.integrate(data, weights, 1, 0, 0, self.tau)

            # goal

            weights = torch.zeros((self.N))
            grad[:, j + self.Dof] = DMP_layer.integrate(data, weights, 0, 0, 1, self.tau)
...
class Net(nn.Module):
    ...
    def __init__(self,scale):
        ...
        self.parameters = createDMPparam(25, 3, 0.01, 2, scale)
